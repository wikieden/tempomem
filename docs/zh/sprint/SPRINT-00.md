...
**目标：** 在 Mac（无 CUDA）上全新安装后 `import tempomem` 可正常运行，schema 与 dataclass 可往返序列化，50 行伪检测数据演示跑通。无真实感知，无融合智能——仅搭好框架。

**完成标准（来自 [roadmap](../03-ROADMAP.md)）：** `pytest -q` 全绿；在干净的 Python 3.11 venv 中 `import tempomem` 正常；演示 notebook 生成可查询的 store。
...
| T10 | `cli.py` — `tempomem inspect <file>` | 计数 + 样本节点 | T6 | 20 min |
...
- [x] `tempomem inspect kitchen.smem` 显示节点计数
