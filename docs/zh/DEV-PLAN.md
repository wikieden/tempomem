...
这是 SpatialRobot 工作区（三个包：`tempomem` / `worldsense` / `mindloop`）的**唯一执行追踪文件**。
...
- **架构已完成拆分**（这是已完成的事，而非未来计划）：perception 位于配套的
  `worldsense`（`BoxDetectorAdapter` + `Detector3D` 接缝 +
  cam→world 几何变换 + `ImageEncoder`）；brain 位于 `mindloop`
...
Chronotope 存储并查询空间记忆。**识别不是我们的职责** —
...
`mindloop/src/mindloop/brain.py:61`（修复前） — 文档字符串写着
...
| # | 任务 | 包 | GPU | 理由（证据） |
|---|---|---|---|---|
| B-T1 | **闭环** — 把 Brain2Robot 重构进 `mindloop`：`Brain.ask()` 从只答题升级为 提出动作 → 执行 → 把结果写回记忆 | brain | no | 没有闭环的大脑只是 QA 机器人；重构目标 2026-06-09 已定 |
...
| B-T4 | **记忆 MCP 化** — 把 `tempomem` 的 query/answer/commit 暴露为 MCP server | core | no | RoboOS issue #76/#73 是对开源场景图记忆组件的活需求；卡住生态位 |
...
| ID | 任务 |
|---|---|
| D1 | ROS 2 桥接节点（订阅 RGB-D，发布 `/tempomem/scene_graph`） |
| D2 | 在共享开放词汇数据集上运行 eMEM 基准测试套件（复用 `bench`） |
| D3 | 摘掉 alpha 标签：发布 `v0.1.0`，撰写发布公告，完成首篇外部集成文章 |
| D4 | 3D Web 查看器（升级 2D 版 `tempomem viz`） |
