...
6. **CLI:** `spatialmem inspect demo.smem` (counts, sample nodes, schema version).
...
| ID | Check | Tool |
|---|---|---|
| A1 | `pip install spatialmem` on macOS arm64 + Linux x86_64 (Python 3.10/3.11/3.12) | CI matrix |
...
| A7 | No hard dep on Torch / CUDA / ROS in default install | `pip show spatialmem` deps audit |
...
| ConceptGraphs install pain leaks into core | high | Hard rule: ConceptGraphs lives behind `pip install spatialmem[conceptgraphs]` |
